...
namespace Thong_Tin_Khach_hang
{
    public partial class frmthongTinKhachHang : Form
    {
        ...
        private void iconbtnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = "SELECT MaLoaiTK from LOAITIETKIEM where TenLoaiTK='" + cboloaiTietKiem.Text + "'";
                dt = dataProvider.Instance.ExecuteQuery(query);
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (index >= 0)
            {
                ...
                txtsdt.Text = dataGridView1.Rows[index].Cells[5].Value.ToString();
                txtgmail.Text = dataGridView1.Rows[index].Cells[6].Value.ToString();
                cboChiNhanh.Text = dataGridView1.Rows[index].Cells[7].Value.ToString();
                cbonhanVien.Text = dataGridView1.Rows[index].Cells[8].Value.ToString();
                dtmNgayThamGia.Text = dataGridView1.Rows[index].Cells[9].Value.ToString();
            }
        }

        private string maNV()
        {
            string query = "SELECT MaNV from NHANVIEN where TenNV='" + cbonhanVien.Text + "'";
            dt = dataProvider.Instance.ExecuteQuery(query);
        ...
        private string maCN()
        {
            string query = "SELECT MaCN from CHINHANH where TenCN='" + cboChiNhanh.Text + "'";
            dt = dataProvider.Instance.ExecuteQuery(query);
        ...
        {
            string query = "SELECT MaNV from NHANVIEN where TenNV='" + cbonhanVien1.Text + "'";
            dt = dataProvider.Instance.ExecuteQuery(query);
            string maNV = dt.Rows[0]["MaNV"].ToString();
            return maNV;
        }
        ...
        {
            string query = "SELECT MaCN from CHINHANH where TenCN='" + cbochiNhanh1.Text + "'";
            dt = dataProvider.Instance.ExecuteQuery(query);
            string TenCN = dt.Rows[0]["MaCN"].ToString();
            return TenCN;
        }
        ...
        {
            string query = "SELECT LaiXuat from LOAITIETKIEM where TenLoaiTK='"+cboloaiTietKiem.Text+"'";
            dt = dataProvider.Instance.ExecuteQuery(query);
        ...
        private void printDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawImage(memoryimg,(60+pagearea.Width/2)-(this.pnlPrint.Width/2), this.pnlPrint.Location.Y);
        }
